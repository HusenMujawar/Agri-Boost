...
<body>
		...
		<div class="sidenav" style="overflow-y:scroll">
			...
			<button class="dropdown-btn">Fertilzer
			<i class="down"></i>
			</button>
	...
	<?php	
		...
		include "config.php";
		$qry="SELECT c_fname FROM customer";
		$result= $conn->query($qry);
		echo mysqli_error($conn);
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				echo "<option>".$row["c_fname"]."</option>";
			}
		}
	?>
	...
	?>
		...
		<form method="post">
			...
			<option value="0" selected="selected">Select Fertilizer</option>
	...
	?>
			...
			<form method="post">
					...
					<label for="medid">Fertilizer ID:</label>
					<input type="number" name="medid" value="<?php echo $row4[0]; ?>"readonly ><br><br>
					
					<label for="mdname">Fertilizer Name:</label>
					<input type="text" name="mdname" value="<?php if(isset($_POST['search'])){ echo $row4[1];} ?>" readonly><br><br>
					
					</div>
					<div class="column">
					
					<label for="mcat">Category:</label>
					<input type="text" name="mcat" value="<?php if(isset($_POST['search'])){ echo $row4[3];} ?>" readonly><br><br>
					
					<label for="mloc">Location:</label>
					<input type="text" name="mloc" value="<?php  if(isset($_POST['search'])){ echo $row4[5];} ?>" readonly><br><br>
					...
					&nbsp;&nbsp;&nbsp;
					<input type="submit" name="add" value="Add Fertilizer">&nbsp;&nbsp;&nbsp;
